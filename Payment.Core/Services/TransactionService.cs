...
namespace Payment.Core.Services
{
    public class TransactionService : ITransactionService
    {
        ...
        {
            var x = !string.IsNullOrWhiteSpace(request.AccountNumber);
            ...
            var eboobe = await VerifyTransferAsync(InitiateRecipientTransfer.Data.reference);

            _logger.Information("Transfer Initiated when specific  recipient from list is not null");
            return ResponseDto<string>.Success("Transaction processing.", $"Transfer from {wallet.Name}");
        }
        ...
        {
            var transactions = _unitOfWork.Transactions.GetTransactionHistory(walletId);
            if (transactions == null)
            {
                return ResponseDto<PaginationResult<IEnumerable<TransactionHistoryResponseDto>>>.Fail
        ...
        private TransferResponseDto TransferResponse(decimal amount, string transferCode, string status, string source, string reference, string createdAt, string updatedAt,
            string message, string narration )
        {
